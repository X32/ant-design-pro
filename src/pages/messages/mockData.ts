import { Session, Message, MessageContent, SessionStatus, MessageRole, MessageType } from './types';
...
// 生成随机时间
const generateRandomTime = (daysAgo: number = 30): string => {
  const date = new Date();
...
// 生成模拟会话数据
export const mockSessions: Session[] = Array.from({ length: 20 }, (_, index) => ({
...
// 生成模拟消息内容数据
export const mockMessageContents: Record<string, MessageContent[]> = {};
...
export const mockApi = {
  // 获取会话列表
  getSessions: (params: { search?: string; filter?: any; page?: number; pageSize?: number }) => {
    return new Promise<{ data: Session[]; total: number }>((resolve) => {
      setTimeout(() => {
  ...
  // 获取消息列表
  getMessages: (sessionId: string, params: { search?: string; filter?: any; page?: number; pageSize?: number }) => {
    return new Promise<{ data: Message[]; total: number }>((resolve) => {
      setTimeout(() => {
  ...
  // 获取消息内容
  getMessageContents: (messageId: string) => {
  ...
  // 删除消息内容
  deleteMessageContent: (contentId: string) => {
    return new Promise<void>((resolve) => {
      setTimeout(() => {
  ...
  // 添加消息内容
  addMessageContent: (messageId: string, content: Omit<MessageContent, 'id' | 'messageId' | 'createdAt'>) => {
    return new Promise<MessageContent>((resolve) => {
      setTimeout(() => {
  ...
  // 更新消息内容
  updateMessageContent: (contentId: string, updates: Partial<Omit<MessageContent, 'id' | 'messageId' | 'createdAt'>>) => {
    return new Promise<MessageContent>((resolve) => {
      setTimeout(() => {
        let updatedContent: MessageContent | undefined;
        Object.keys(mockMessageContents).forEach(messageId => {
          const contentIndex = mockMessageContents[messageId].findIndex(c => c.id === contentId);
          if (contentIndex !== -1) {
