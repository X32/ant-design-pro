...
import { history, useSearchParams } from '@umijs/max'; // 导入useSearchParams用于获取URL参数
// 导入所需的Ant Design组件
import { Button, Card, Input, Layout, Modal, Space } from 'antd';
...
import {
  ...
  TOKEN_KEY,
  USER_ID_KEY,
} from '@/config/apiConfig'; // 导入Token键名常量
// 导入AudioRecorder06组件
...
interface Message {
  ...
  content:
    ...
    | FinalScoreSummaryContent; // 消息内容（文本、评分、语法反馈或总分汇总对象）
  ...
  messageType?:
    ...
    | 'final_score_summary'
    | 'finish'
    | 'score_panel'; // 消息类型（新增 score_panel 类型）
...
// AI音频基础URL
// const AI_AUDIO_BASE_URL = isDev ? 'http://localhost:9002' : 'https://www.speakcube.cn';
const AI_AUDIO_BASE_URL = 'http://localhost:9002';
// const AI_AUDIO_BASE_URL = 'https://www.speakcube.cn';
...
const SpokenPractice: React.FC = () => {
  ...
  // 工作流类型价格映射表（从接口动态获取）
  const [workflowPriceMap, setWorkflowPriceMap] = useState<Map<string, number>>(
    new Map(),
  );
  // 💡 新增：使用 Ref 存储价格映射的副本，确保实时可用（解决状态更新异步问题）
  const workflowPriceMapRef = useRef<Map<string, number>>(new Map());
  const [_priceLoading, setPriceLoading] = useState(false);
  ...
  // 录音文件状态管理
  const [_recordedFile, setRecordedFile] = useState<string | null>(null);
  ...
  // 🆕 语法反馈弹窗状态管理
  const [grammarFeedbackModalVisible, setGrammarFeedbackModalVisible] =
    useState(false);
  const [currentGrammarFeedback, setCurrentGrammarFeedback] =
    useState<GrammarFeedbackContent | null>(null);
  ...
  const [expectedTotalParts, setExpectedTotalParts] = useState<number | null>(
    null,
  ); // 总 part 数
  ...
  const [_vipLoading, setVipLoading] = useState(false);
   ...
   */
  const fetchExamPaperQuestionsAndPrice = async (
    paperId: number,
  ): Promise<{
    totalPrice: number;
    ...
    try {
      ...
      questions.forEach((question: any) => {
        ...
        const workflowType =
          question.workflow_type || question.exercise?.workflow_type;
        ...
        if (workflowType) {
          ...
          console.log(
            `  题目 #${question.id}: workflow_type=${workflowType}, price=${price}`,
          );
        } else {
      ...
      questions.forEach((question: any, index: number) => {
        ...
        if (exerciseId && workflowType) {
          ...
          if (partMatch?.[1]) {
            ...
            console.log(
              `  题目 #${question.id}: workflow_type=${workflowType} → key="${partNumber}", exercise_id=${exerciseId}`,
            );
          } else {
            // 如果没有 part 数字，使用索引 + 1 作为 key
            exerciseIds[String(index + 1)] = exerciseId;
            console.warn(
              `⚠️ 题目 #${question.id} 的 workflow_type "${workflowType}" 没有 part 数字，使用索引: ${index + 1}`,
            );
          }
        } else {
          if (!exerciseId) {
  ...
  const getWorkflowPrice = (workflowType: string): number => {
    ...
    const priceMap =
      workflowPriceMapRef.current.size > 0
        ? workflowPriceMapRef.current
        : workflowPriceMap;
    const price = priceMap.get(workflowType);

    if (price === undefined) {
      console.warn(
        `⚠️ 未找到 workflow_type "${workflowType}" 的价格，返回默认值 0`,
      );
  ...
  const checkBalance = async (
    ...
    try {
      ...
      if (hasVipSubscription && vipRemainingDays > 0) {
        console.log(
          `💎 VIP用户特权：跳过金币检查（剩余${vipRemainingDays}天）`,
        );
        return {
          sufficient: true,
          balance: 0,
      ...
      if (response.success && response.data) {
        ...
        console.log(
          `💰 用户余额: ${balance} 金币, 需要: ${price} 金币, 充足: ${sufficient}`,
        );
  ...
  const handleConsumeCoins = async (): Promise<boolean> => {
    try {
      ...
      console.log(
        `💳 开始扣款: paper_id=${paperId}, workflow_type=${workflowType}, price=${price}`,
      );
      ...
      if (response.success) {
        ...
        console.log(
          `💰 余额变化: ${response.data.balance_before} → ${response.data.balance_after}`,
        );
  ...
  const handleFinishConsumeCoins = async (): Promise<void> => {
    ...
    if (!balanceCheck.sufficient) {
      console.warn(
        `⚠️ 余额不足: 当前余额=${balanceCheck.balance}, 需要=${balanceCheck.price}`,
      );
      ...
      Modal.warning({
        ...
        content: (
          <div>
            ...
            <p style={{ marginTop: '12px' }}>
              <span style={{ color: '#ff4d4f', fontWeight: 'bold' }}>
                当前余额：{balanceCheck.balance} 金币
              </span>
            </p>
            <p>
              <span style={{ color: '#1890ff', fontWeight: 'bold' }}>
                需要金币：{balanceCheck.price} 金币
              </span>
            </p>
            <p style={{ marginTop: '12px', color: '#666' }}>
              请充值后再进行练习。
  ...
  const saveSingleMessage = async (
    ...
    console.log(
      `💾 [🔹 conversationId: ${conversationId}] [🔹 messageId: ${msg.id}] [🔹 messageType: ${msg.messageType}] 准备保存消息 [${index + 1}/${total}], sender: ${msg.sender}, created_at: ${createdAt}`,
    );
    ...
    try {
      ...
      switch (msg.messageType) {
        ...
        case 'grammar_feedback': {
          ...
          if (
            workflowType.toLowerCase().includes('ket') ||
            workflowType.toLowerCase().includes('a2')
          ) {
            examLevel = 'KET';
          } else if (
            workflowType.toLowerCase().includes('pet') ||
            workflowType.toLowerCase().includes('b1')
          ) {
            examLevel = 'PET';
          } else if (
            workflowType.toLowerCase().includes('fce') ||
            workflowType.toLowerCase().includes('b2')
          ) {
            examLevel = 'FCE';
          }

          console.log(
            `📝 准备保存语法分析消息, exam_level: ${examLevel}, workflowType: ${workflowType}`,
          );
          ...
          if (examLevel === 'KET' && grammarContent.a2_assessment) {
            assessmentJson = JSON.stringify({
              a2_grammar_structure:
                grammarContent.a2_assessment.grammar_structure,
            ...
            assessmentJson = JSON.stringify({
              b1_grammar_structure:
                grammarContent.b1_assessment.grammar_structure,
          ...
          return await createGrammarAnalysisMessage(conversationId, {
            origin_message_id: msg.originMessageId
              ? msg.originMessageId.toString()
              : undefined, // 🆕 使用关联的音频消息ID
      ...
      console.error(
        `❌ 保存${msg.messageType || 'text'}消息失败 [${index + 1}/${total}]:`,
        err,
      );
      return null;
    }
  ...
  const handleGrammarFeedbackMessage = (
    ...
    try {
      ...
      console.log(
        `🔗 关联语法反馈到用户消息, origin_message_id: ${originMessageId}`,
      );
      ...
      console.log(
        `💾 Grammar 消息已添加到缓存, grammarMessageId: ${grammarMessageId}, originMessageId: ${originMessageId}`,
      );
      ...
      setMessages((prevMessages) => {
        const updated = prevMessages.map((msg) => {
          if (msg.id === originMessageId) {
            console.log(
              `✅ 找到匹配的用户消息, id: ${msg.id}, type: ${msg.messageType}, 错误数: ${grammarFeedback.errors?.length || 0}`,
            );
            return {
              ...msg,
        ...
        if (!found) {
          console.warn(
            `⚠️ 未找到 id=${originMessageId} 的消息，但 grammar 消息已缓存`,
          );
        }
  ...
  const handleTextMessage = (
    ...
    const textMessage: Message = {
      id: messageId,
      content:
        typeof data.content === 'string' ? data.content : '我收到了你的消息!',
      sender: 'ai',
      timestamp: now.toLocaleTimeString([], {
        hour: '2-digit',
        minute: '2-digit',
  ...
  const handleScoreMessage = (
    ...
    console.log('🔍 评分消息解析结果:', {
      ...
      优势: parsedContent.advantages
        ? `有(${parsedContent.advantages.length}字)`
        : '无',
      不足: parsedContent.disadvantages
        ? `有(${parsedContent.disadvantages.length}字)`
        : '无',
      改进建议: parsedContent.suggestions
        ? `有(${parsedContent.suggestions.length}字)`
        : '无',
      改进的回答: parsedContent.improvedAnswer
        ? `有(${parsedContent.improvedAnswer.length}字)`
        : '无',
    });
    ...
    if (data.part_no !== undefined && data.total_parts !== undefined) {
      ...
      console.log(
        `📋 评分消息已缓存: Part ${data.part_no}/${data.total_parts}, 当前已收到 ${scoreMessagesCache.current.size} 个`,
      );
  ...
  const connectWebSocket = async (realConversationId: number) => {
    try {
      ...
      const { workflowTypes, totalPrice, exerciseIds } =
        await fetchExamPaperQuestionsAndPrice(paperId);
      ...
      wsSocket.on(
        'retrying',
        (data: { attempt: number; maxRetries: number; interval: number }) => {
  ...
  const loadHistoryMessages = async (
    ...
    try {
      ...
      if (response.success && response.data && response.data.length > 0) {
        console.log(
          `✅ 找到 ${response.data.length} 条历史消息，继续使用会话 ID: ${conversationId}`,
        );
  ...
  const [conversationId] = useState<number | null>(() =>
   ...
   */
  const preloadAndPlayAiAudio = (messageId: string, audioUrl: string) => {
    // 🔧 修改为 string 类型
    // 停止当前正在播放的音频
    if (audioElement) {
      audioElement.pause();
    ...
    const initializeConversation = async () => {
      try {
        ...
        if (vipResult.status === 'rejected') {
          console.warn(
            '⚠️ VIP订阅状态加载失败，将使用默认值:',
            vipResult.reason,
          );
        }

        // 统计成功/失败数量
        const successCount = configResults.filter(
          (r) => r.status === 'fulfilled',
        ).length;
        console.log(
          `📊 配置加载完成: ${successCount}/${configResults.length} 成功`,
        );
   ...
   */
  const sendUserMessageToAPI = async (
   ...
   */
  const _handleApiMessageSend = async (
    conversationId: number,
    userId: number,
    content: string,
    seq: number,
    ...
    try {
      // 调用API发送消息到后端
      const apiResponse = await sendUserMessageToAPI(
        conversationId,
        userId,
        content,
        seq,
              ...
              if (transcriptionText) {
                ...
                if (isConnected && socket) {
                  try {
                    ...
                    console.log(
                      '已将转写结果通过WebSocket发送, origin_message_id:',
                      newMessageId,
                    );
                    ...
                    messageCacheRef.current = messageCacheRef.current.map(
                      (msg) =>
                        msg.id === newMessageId
                          ? { ...msg, grammarFeedbackStatus: 'pending' }
                          : msg,
  ...
  const playAudioMessage = (message: Message) => {
    ...
    const audioSrc = message.audioFilePath;
   ...
   * 🆕 打开语法反馈弹窗
   */
  const openGrammarFeedbackModal = (
        ...
        <div className="header-left">
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (aiAudioCacheRef.current.size > 0) {
                console.log(
                  `🧹 清理 ${aiAudioCacheRef.current.size} 个缓存的音频`,
                );
                aiAudioCacheRef.current.forEach((audio) => {
                  audio.pause();
                  audio.src = '';
                ...
                message.audioFilePath ? (
                      ...
                      message.grammarFeedbackStatus === 'received' && (
                        ...
                        >
                          <Button
                            ...
                            onClick={() =>
                              openGrammarFeedbackModal(message.grammarFeedback!)
                            }
                            style={{
                              backgroundColor: '#52c41a',
                              borderColor: '#52c41a',
                          ...
                          >
                            查看语法反馈 (
                            {message.grammarFeedback.errors?.length || 0}{' '}
                            个错误)
                          </Button>
                        </div>
                      )}
                  ...
                  >
                    <img
                      ...
                      style={{
                        width: expandedImages.has(message.id)
                          ? '100%'
                          : '300px',
                        maxWidth: '100%',
                        height: 'auto',
                  ...
                  message.messageType === 'score_panel' ? (
                  /* 🆕 评分面板消息 */
                  <div
                    className="ai-score-panel-content"
                    style={{
                      padding: '20px',
                  ...
                  >
                    ...
                    {message.scoreParts && message.scoreParts.length > 0 && (
                      ...
                      >
                        {message.scoreParts.map((scorePart, _index) => (
                          <div
                            key={scorePart.id}
                            style={{
                          ...
                          >
                            ...
                            {/* Part 内容 */}
                            {typeof scorePart.content === 'object' &&
                              scorePart.content !== null && (
                                <div
                                  style={{
                                    fontSize: '14px',
                                    lineHeight: '1.6',
                                ...
                                >
                                  {/* 维度分数 */}
                                  {(scorePart.content as ScoreContent)
                                    .dimensionScores && (
                  ...
                  message.messageType === 'final_score_summary' ? (
                  /* 🏆 总分汇总消息 */
                  <div
                    className="ai-final-score-summary-content"
                    style={{
                      padding: '24px',
                      backgroundColor: '#f6ffed',
                      borderRadius: '12px',
                  ...
                  >
                    ...
                    {/* 分数详情 */}
                    {typeof message.content === 'object' &&
                      'part_scores' in message.content && (
                        <div
                          style={{
                            display: 'flex',
                            flexDirection: 'column',
                        ...
                        >
                          {/* 各 Part 得分 */}
                          {Object.entries(
                            (message.content as FinalScoreSummaryContent)
                              .part_scores,
                          ).map(([part, score]) => (
                            <div
                              key={part}
                  ...
                  /* 🏁 对话结束消息 */
                  <div
                    className="ai-finish-message-content"
                    style={{
                      padding: '20px',
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        console.log(
                          '👆 用户点击开始新对话，清除 conversationId 并刷新页面',
                        );
                        localStorage.removeItem(CONVERSATION_ID_KEY);
                        window.location.reload();
                      }}
          ...
          {showTextInput && (
            <div style={{ marginBottom: '12px' }}>
              <TextArea
                ...
                placeholder={
                  conversationFinished ? '对话已结束' : '输入内容...'
                }
                rows={3}
              ...
              <Button
                ...
                disabled={
                  conversationFinished || !isConnected || !isAuthenticated
                } // 🚫 finish 后禁用
                title={
      ...
      <Modal
        ...
        footer={[
          <Button
            key="close"
            type="primary"
            onClick={closeGrammarFeedbackModal}
          >
            关闭
          </Button>,
        ]}
      ...
      >
        {currentGrammarFeedback && (
          <div style={{ maxHeight: '70vh', overflowY: 'auto' }}>
              ...
              <div style={{ fontSize: '14px' }}>
                ...
                <span
                  style={{
                    ...
                    borderRadius: '4px',
                    backgroundColor:
                      currentGrammarFeedback.overall_quality === 'excellent'
                        ? '#52c41a'
                        : currentGrammarFeedback.overall_quality === 'good'
                          ? '#1890ff'
                          : currentGrammarFeedback.overall_quality === 'fair'
                            ? '#faad14'
                            : '#ff4d4f',
                    color: '#fff',
                    fontWeight: 'bold',
              ...
              <div style={{ fontSize: '14px', marginBottom: '8px' }}>
                ...
                <span
                  style={{
                    ...
                    borderRadius: '4px',
                    backgroundColor:
                      currentGrammarFeedback.relevance_level === 'on_topic'
                        ? '#52c41a'
                        : currentGrammarFeedback.relevance_level ===
                            'partially_on_topic'
                          ? '#faad14'
                          : '#ff4d4f',
                    color: '#fff',
                ...
                <span style={{ marginLeft: '8px', color: '#666' }}>
                  (分数: {currentGrammarFeedback.relevance_score.toFixed(2)})
                </span>
              </div>
              <div
            ...
            {/* 错误列表 */}
            {currentGrammarFeedback.errors &&
              currentGrammarFeedback.errors.length > 0 && (
                <Card
                  size="small"
                  title={
                    <span style={{ color: '#ff4d4f', fontWeight: 'bold' }}>
                  ...
                  {currentGrammarFeedback.errors.map((error, index) => (
                    <div
                      ...
                      style={{
                        padding: '12px',
                        marginBottom:
                          index < currentGrammarFeedback.errors.length - 1
                            ? '12px'
                            : '0',
                        backgroundColor:
                          error.severity === 'critical' ? '#fff1f0' : '#fffbe6',
            ...
            {/* 学习建议 */}
            {currentGrammarFeedback.suggestions &&
              currentGrammarFeedback.suggestions.length > 0 && (
                <Card
                  size="small"
                  title={
                    <span style={{ color: '#1890ff', fontWeight: 'bold' }}>
                ...
                >
                  <ul style={{ margin: 0, paddingLeft: '20px' }}>
                    {currentGrammarFeedback.suggestions.map(
                      (suggestion, index) => (
                        <li
                          key={index}
                          style={{
            ...
            {(currentGrammarFeedback.a2_assessment ||
              ...
              <Card
                size="small"
                title={<span style={{ fontWeight: 'bold' }}>📊 维度评估</span>}
              >
                {currentGrammarFeedback.a2_assessment && (
                  <div style={{ marginBottom: '8px' }}>
                    ...
                    >
                      语法:{' '}
                      {currentGrammarFeedback.a2_assessment.grammar_structure} |
                ...
                {currentGrammarFeedback.b1_assessment && (
                  <div style={{ marginBottom: '8px' }}>
                    ...
                    >
                      语法:{' '}
                      {currentGrammarFeedback.b1_assessment.grammar_structure} |
                ...
                {currentGrammarFeedback.b2_assessment && (
                  <div>
                    ...
                    >
                      语法:{' '}
                      {currentGrammarFeedback.b2_assessment.grammar_structure} |
